...
export const createPrivate = catchAsync(async (req, res, next) => {
  ...
  if (isExist) {
    return res
      .status(400)
      .json({ message: "chat already exists", data: { chatId: oldChatId } });
  }
  if (myChat?.length == 0) {
  ...
  const chat = await Chat.findByIdAndUpdate(myChat[0]._id, {
    $push: { members: req.userId },
  });
  await User.findByIdAndUpdate(
    req.userId,
    { $push: { chatList: chat[0]._id } },
    { new: true }
  );
  res.status(200).json({
    success: true,
...
export const update = catchAsync(async (req, res, next) => {
  const { chatId = null, chatName = null, name = null } = req.body;
  if (!chatId) {
    return next(new HandleERROR("chat ID is required", 400));
  }
  const chat = await Chat.findById(chatId);
  if (!chat) {
    return next(new HandleERROR("chat not found", 404));
  ...
  const newChat = await chat.save();
  const socketId = getSocketId(chat.members);
  if (socketId.length > 0) {
    socketId.forEach((id) => {
      io.to(id).emit("chatUpdated", newChat);
    });
  }
  return res.status(200).json({
    success: true,
...
export const removePrivate = catchAsync(async (req, res, next) => {
  ...
  if (chat.type == "private") {
    return next(new HandleERROR("cannot remove public chat", 400));
  }
  await User.findByIdAndUpdate(req.userId, { $pull: { chatList: chatId } });
  if (chat.members.length == 2) {
    const removedMembers = chat.removeMembers?.filter(
      (e) => e.userId != req.userId
    );
    chat.removeMembers = removedMembers;
    chat.removeMembers.push({
      userId: req.userId,
      removeAt: new Date(),
    });
    await chat.save();
    return res.status(200).json({
      success: true,
      message: "private chat removed successfully",
    });
  }
